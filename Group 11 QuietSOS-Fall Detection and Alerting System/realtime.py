...
while True:
    ...
    if model_active:
        ...
        # =========================================================
        # RESET BACK TO PREFILTER (ONLY ADDITION)
        # =========================================================

        if model_frame_counter >= LOOKBACK_FRAMES and not fall_candidate_active:
            print("\n[INFO] 60-frame evaluation complete. No fall confirmed. Returning to PREFILTER.")
            model_active = False
            prefilter_active = True
            pose_buffer.clear()
            last_fall_prob = 0.0
        ...
        if alert_sent and is_standing:
            print("\n[INFO] Person recovered. Resetting system to PREFILTER.")
            model_active = False
            prefilter_active = True
            alert_sent = False
            fall_candidate_active = False
    ...
    if alert_sent:
        ...
        cv2.putText(display_frame, "CRITICAL ALERT: EMERGENCY CONFIRMED",
                    (w // 2 - 250, h - 20), font, 0.9, (255, 255, 255), 3)

    sys.stdout.write(f"\r[LIVE FEED] Mode: {term_state:<28} | Fall Prob: {term_prob:.2f} | Ground Status: {term_ground:<10}")
    sys.stdout.flush()

    cv2.namedWindow("QuietSOS Live Monitor", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("QuietSOS Live Monitor", w, h)
